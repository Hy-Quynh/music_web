...
const columns = [
  { id: "stt", label: "#", minWidth: 150, align: "center" },
  {
    id: "name",
    label: "Name",
    minWidth: 170,
...
export default function AdminCategory() {
  ...
  const getlistCategory = async () => {
    try {
      const branchRes = await getAllCategory();
  ...
  const handleCreateUpdateCategory = async () => {
    ...
    if (!categoryName.trim().length || !description.trim().length) {
      ...
      if (addCategoryModal.type === "add") {
        const createBranchRes = await createNewCategory(
          categoryName,
          description
        );
        if (createBranchRes?.data?.success) {
          ...
          return toast.error(
            createBranchRes?.data?.error || "Add new category failed"
          );
        }
      } else {
